...
class TRAJECTORY(object):
    ...
    def __init__(self, sim_time, trajName):
        ...
        # x, y, z, r, p, y
        self.obs_low = np.array([-np.inf, -np.inf, -np.inf, -np.pi, -np.pi, -np.pi])
        self.obs_low = np.array([np.inf, np.inf, np.inf, np.pi, np.pi, np.pi])
        #
        self.reset()
    ...
    def run(self, t):
        ...
        if self.trajName == "cycloid":
            r = np.array(
                [np.sqrt(t) * np.sin(t), np.sqrt(t) * np.cos(t), 0.5 * t, 0, 0, 0]
            )

        elif self.trajName == "hover":
            T = self.sim_T
            if t <= T / 3:
                r = np.array([0, 0, t, 0, 0, 0])
            else:
                r = np.array([0, 0, T / 3, 0, 0, 0])
